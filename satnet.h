...
class SatNet{
    ...
    // ***************************************************
  Sat* helperInsert(Sat* node, Sat* nodeNew);
  Sat* rotateLeft(Sat* node);
  Sat* rotateRight(Sat* node);
  void helperClear(Sat* node);
  int  getBalance(Sat* node);
  Sat* minValue(Sat* node);
  Sat* helperRemove(Sat* root, int ID);
  void helperListSatellites(Sat* node) const;  
  bool helperSetState(Sat* node, int id, STATE satelliteState);
  Sat* helperRemoveDeorbited(Sat* node);
  bool findSatelliteHelper(Sat* node, int id) const;
  int helperCountSatellites(Sat* node, INCLIN treeDegree) const; 
